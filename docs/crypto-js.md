...
`crypto-js` is no longer actively maintained and has been discontinued since engines now come with this functionality
built-in.
...
The [Web Crypto API](https://developer.mozilla.org/docs/Web/API/Web_Crypto_API) provides native functionality for
cryptographic operations in both web browsers and Node.
...
Bun supports the Web Crypto API natively, and also provides support for streaming hashing via [
